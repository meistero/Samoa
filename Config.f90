...
#    	if defined(_DARCY)
            write(arguments, '(A, A)') trim(arguments), "  -dmin 1 -dmax 14 -tsteps -1 -tmax 2.0e1 -tout -1.0 -fperm data/darcy_benchmark/perm.nc"
...
#				if defined(_ASAGI_NUMA)
                	_log_write(0, '(" ASAGI: Yes, with NUMA support, mode: ", I0)') cfg%i_asagi_mode
#				else
                	_log_write(0, '(" ASAGI: Yes, without NUMA support, mode: ", I0)') cfg%i_asagi_mode
#				endif
#    		else
...
#    		endif
            ...
            _log_write(0, '(" Sections per thread: ", I0)') cfg%i_sections_per_thread
            _log_write(0, '(" Adaptivity: min depth: ", I0, ", max depth: ", I0)') cfg%i_min_depth, cfg%i_max_depth
            _log_write(0, '(" Simulation: max time steps: ", I0, ", max time: ", ES9.2, ", output step: ", ES9.2)'), cfg%i_max_time_steps, cfg%r_max_time, cfg%r_output_time_step

#    		if defined(_DARCY)
                _log_write(0, '(" Scenario: permeability template: ", A)') trim(cfg%s_permeability_file)
#    		elif defined(_SWE)
                _log_write(0, '(" Scenario: bathymetry file: ", A, ", displacement file: ", A)') trim(cfg%s_bathymetry_file), trim(cfg%s_displacement_file)
#    		endif
...
#		if defined(_ASAGI)
			select case(cfg%i_asagi_mode)
				case (0)
					cfg%i_asagi_mode = GRID_NO_HINT
				case (1)
					cfg%i_asagi_mode = ieor(GRID_NOMPI, GRID_PASSTHROUGH)
				case (2)
					cfg%i_asagi_mode = GRID_NOMPI
				case (3)
					cfg%i_asagi_mode = ieor(GRID_NOMPI, SMALL_CACHE)
				case (4)
					cfg%i_asagi_mode = GRID_LARGE_GRID
			end select
#		endif
