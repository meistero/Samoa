...
#		include "SFC_generic_traversal_ringbuffer.f90"
        ...
        subroutine pre_traversal_grid_op(traversal, grid)
            ...
            if (cfg%l_ascii_out) then
                try (cfg%i_ascii_width >= 2, "Invalid AsciiOutput width") 
                ascii = create_ascy(cfg%i_ascii_width, (cfg%i_ascii_width/2), dble(0.001), dble([cfg%scaling, cfg%scaling]), dble(cfg%offset))
            endif
