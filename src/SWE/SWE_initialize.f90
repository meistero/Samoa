...
!                        element%nodes(2)%ptr%data_pers%is_dirichlet_boundary=.true.
		...
		elemental subroutine inner_node_first_touch_op(traversal, section, node)
            ...
            real (kind=GRID_SR), parameter                       ::acc=0.01_GRID_SR
            !beach
...
!                    !write (*,*) -eta_xs *( u_xp-u_xm /(2.0_GRID_SR*acc))
...
#			else
                ...
                elseif (cfg%s_test_case_name .eq. 'bar') then
                     ...
                     Q%p(1)=0.0_GRID_SR
                     Q%p(2)=0.0_GRID_SR
                else if (cfg%s_test_case_name .eq. 'sea_rest') then
                     Q%h=0.0_GRID_SR
                     Q%p(1)=0.0_GRID_SR
                     Q%p(2)=0.0_GRID_SR
