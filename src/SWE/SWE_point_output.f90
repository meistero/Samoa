...
#include "Compilation_control.f90"
	...
	MODULE SWE_point_output
		...
		double precision, allocatable		:: r_testpoints(:,:)
...
#     endif      
	...
	subroutine element_op(traversal, section, element)
		...
		double precision 						:: h, b, p(2), local_coord(2), epsvec(2), eps, distvec(2), dist
		...
		do i=1, size(r_testpoints, dim=1)
			...
			if (local_coord(1) + eps >= 0.0_GRID_SR .and. local_coord(2) + eps >= 0.0_GRID_SR .and. local_coord(1) + local_coord(2) - eps <= 1.0_GRID_SR) then
                ...
                distvec = dble(samoa_barycentric_to_world_point(element%transform_data, [1.0_GRID_SR/3.0_GRID_SR, 1.0_GRID_SR/3.0_GRID_SR])) - [r_testpoints(i,1), r_testpoints(i,2)]
                dist = norm2(distvec)
