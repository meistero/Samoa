...
#		define	_GT_NAME							heat_eq_output_traversal
...
#		include "SFC_generic_traversal_ringbuffer.f90"
		...
		subroutine post_traversal_op(traversal, section)
			...
			write (s_file_name, "(A, A, I0, A)") TRIM(grid%s_file_stamp), "_", i_output_iteration, ".vtk"

			e_io = VTK_INI('ASCII', s_file_name, 'Heat Equation', 'UNSTRUCTURED_GRID')
				e_io = VTK_GEO(i_points, point_data%coords(1), point_data%coords(2), r_empty(1:i_points))

				e_io = VTK_CON(i_cells, i_connectivity, i_types)

				e_io = VTK_DAT(i_points, 'node')
				e_io = VTK_VAR(i_points, 'temperature', point_data%T)

				e_io = VTK_DAT(i_cells, 'cell')
				e_io = VTK_VAR(i_cells, 'heat_conductivity', cell_data%heat_conductivity)
				e_io = VTK_VAR(i_cells, 'grid_depth', cell_data%depth)
				e_io = VTK_VAR(i_cells, 'refinement_flag', cell_data%refinement)
			e_io = VTK_END()
		...
		subroutine element_op(traversal, section, element)
			...
			if (l_second_order) then
				...
				i_connectivity(7 * i_cell_data_index - 6) = 6
				i_connectivity(7 * i_cell_data_index - 5 : 7 * i_cell_data_index) = point_data_indices((/ 1, 2, 3, 6, 4, 5 /)) - 1
			else
				forall (i = 1 : 3)
					point_data(point_data_indices(i))%coords = samoa_barycentric_to_world_point(element%transform_data, samoa_basis_T_get_dof_coords(i))
					point_data(point_data_indices(i))%T = T(i)
				...
				i_connectivity(4 * i_cell_data_index - 3) = 3
				i_connectivity(4 * i_cell_data_index - 2 : 4 * i_cell_data_index) = point_data_indices(1:3) - 1
			end if
