...
!
  ...
  allocate ( suborder(1:suborder_num), stat = i_error); assert_eq(i_error, 0)

  call dunavant_suborder ( rule, suborder_num, suborder )

  order_num = sum ( suborder(1:suborder_num) )

  deallocate ( suborder, stat = i_error); assert_eq(i_error, 0)
...
!
  ...
  allocate ( suborder(suborder_num), stat = i_error); assert_eq(i_error, 0)
  allocate ( suborder_xyz(3,suborder_num), stat = i_error); assert_eq(i_error, 0)
  allocate ( suborder_w(suborder_num), stat = i_error); assert_eq(i_error, 0)
...
!
  ...
  deallocate ( suborder, stat = i_error); assert_eq(i_error, 0)
  deallocate ( suborder_xyz , stat = i_error); assert_eq(i_error, 0)
  deallocate ( suborder_w, stat = i_error); assert_eq(i_error, 0)
...
!    Input, integer ( kind = 4 ) RULE, the index of the rule.
!
!    Input, integer ( kind = 4 ) SUBORDER_NUM, the number of suborders of the rule.
!
!    Output, integer ( kind = 4 ) SUBORDER(SUBORDER_NUM), the suborders of the rule.
!
  implicit none
