...
!> #define _BF_ORDER		<Order of the hierarchical basis>
...
#define _BF_EDGE1				4
#define _BF_EDGE1E				_BF_EDGE1 + (_BF_ORDER - 2)
#define _BF_EDGE2				_BF_EDGE1E + 1
#define _BF_EDGE2E				_BF_EDGE2 + (_BF_ORDER - 2)
#define _BF_EDGE3				_BF_EDGE2E + 1
#define _BF_EDGE3E				_BF_EDGE3 + (_BF_ORDER - 2)
#define _BF_CELL				_BF_EDGE3E + 1
...
#	define _DPSI_DY(x, y) 	[ 0.0_GRID_SR ]
	...
	real (kind = GRID_SR), DIMENSION(_BF_SIZE, _BF_SIZE), parameter	:: m_phi = reshape( &
		[	1.0_GRID_SR ], &
		[ _BF_SIZE, _BF_SIZE ] )

	real (kind = GRID_SR), DIMENSION(_BF_SIZE, _BF_SIZE), parameter	:: m_phi_inv = m_phi
#elif (_BF_ORDER == 1)
	real (kind = GRID_SR), DIMENSION(2, _BF_SIZE), parameter		:: r_dof_coords = reshape( &
...
#	define _DPSI_DY(x, y) 	[ 0.0_GRID_SR, -1.0_GRID_SR, 1.0_GRID_SR ]
	...
	real (kind = GRID_SR), DIMENSION(_BF_SIZE, _BF_SIZE), parameter	:: m_phi = reshape( &
		[	1.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, &
			0.0_GRID_SR, 1.0_GRID_SR, 0.0_GRID_SR, &
			0.0_GRID_SR, 0.0_GRID_SR, 1.0_GRID_SR ], &
		[ _BF_SIZE, _BF_SIZE ] )
...
#	define _PSI(x, y) 		[ x, 1.0_GRID_SR - x - y, y, 4.0_GRID_SR * y * (1.0_GRID_SR - x - y), 4.0_GRID_SR * x * y, 4.0_GRID_SR * x * (1.0_GRID_SR - x - y) ]
#	define _DPSI_DX(x, y) 	[ 1.0_GRID_SR, -1.0_GRID_SR, 0.0_GRID_SR, -4.0_GRID_SR * y, 4.0_GRID_SR * y, 4.0_GRID_SR - 8.0_GRID_SR * x - 4.0_GRID_SR * y ]
#	define _DPSI_DY(x, y) 	[ 0.0_GRID_SR, -1.0_GRID_SR, 1.0_GRID_SR, 4.0_GRID_SR - 4.0_GRID_SR * x - 8.0_GRID_SR * y, 4.0_GRID_SR * x, -4.0_GRID_SR * x ]

	real (kind = GRID_SR), DIMENSION(_BF_SIZE, _BF_SIZE), parameter	:: m_phi = reshape( &
		[	1.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, 0.5_GRID_SR, 0.5_GRID_SR, &
			0.0_GRID_SR, 1.0_GRID_SR, 0.0_GRID_SR, 0.5_GRID_SR, 0.0_GRID_SR, 0.5_GRID_SR, &
			0.0_GRID_SR, 0.0_GRID_SR, 1.0_GRID_SR, 0.5_GRID_SR, 0.5_GRID_SR, 0.0_GRID_SR, &
			0.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, 1.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, &
			0.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, 1.0_GRID_SR, 0.0_GRID_SR, &
			0.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, 1.0_GRID_SR ], &
		[ _BF_SIZE, _BF_SIZE ] )

	real (kind = GRID_SR), DIMENSION(_BF_SIZE, _BF_SIZE), parameter	:: m_phi_inv = reshape( &
		[	1.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, -0.5_GRID_SR, -0.5_GRID_SR, &
			0.0_GRID_SR, 1.0_GRID_SR, 0.0_GRID_SR, -0.5_GRID_SR, 0.0_GRID_SR, -0.5_GRID_SR, &
			0.0_GRID_SR, 0.0_GRID_SR, 1.0_GRID_SR, -0.5_GRID_SR, -0.5_GRID_SR, 0.0_GRID_SR, &
			0.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, 1.0_GRID_SR, 0.0_GRID_SR, 0.0_GRID_SR, &
		...
		[ _BF_SIZE, _BF_SIZE ] )
#else
#	error: Hierarchical basis must be of order less than 3
#endif
...
!>Evaluates a function at the DoFs positions
pure function _BF_(dofs_to_values)(r_dofs) result(r_values)
	...
	!For linear and constant basis functions: values = DoFs
	r_values(1 : min(3, _BF_SIZE)) = r_dofs(1 : min(3, _BF_SIZE))

	!Edge values are computed from the DoFs of the respective edge and its adjacent nodes
#	if (_BF_ORDER > 1)
		r_values(_BF_EDGE1 : _BF_EDGE1E) = &
			MATMUL(m_phi(_BF_EDGE1 : _BF_EDGE1E, _BF_EDGE1 : _BF_EDGE1E), r_dofs(_BF_EDGE1 : _BF_EDGE1E)) + &
			m_phi(_BF_EDGE1 : _BF_EDGE1E, 2) * r_dofs(2) + &
			m_phi(_BF_EDGE1 : _BF_EDGE1E, 3) * r_dofs(3)

		r_values(_BF_EDGE2 : _BF_EDGE2E) = &
			MATMUL(m_phi(_BF_EDGE2 : _BF_EDGE2E, _BF_EDGE2 : _BF_EDGE2E), r_dofs(_BF_EDGE2 : _BF_EDGE2E)) + &
			m_phi(_BF_EDGE2 : _BF_EDGE2E, 1) * r_dofs(1) + &
			m_phi(_BF_EDGE2 : _BF_EDGE2E, 3) * r_dofs(3)

		r_values(_BF_EDGE3 : _BF_EDGE3E) = &
			MATMUL(m_phi(_BF_EDGE3 : _BF_EDGE3E, _BF_EDGE3 : _BF_EDGE3E), r_dofs(_BF_EDGE3 : _BF_EDGE3E)) + &
			m_phi(_BF_EDGE3 : _BF_EDGE3E, 1) * r_dofs(1) + &
			m_phi(_BF_EDGE3 : _BF_EDGE3E, 2) * r_dofs(2)
#	endif

	!Higher-order cell values depend on all DoFs
#	if (_BF_ORDER > 2)
		r_values(_BF_CELL :) = MATMUL(m_phi(:, _BF_CELL :), r_dofs)
#	endif
end function

!>Returns the DoFs of a function evaluated at the DoF positions
pure function _BF_(values_to_dofs)(r_values) result(r_dofs)
	_BF_type, DIMENSION(:), intent(in)					:: r_values 		!< function values, overwritten by degrees of freedom
	_BF_type, DIMENSION(_BF_SIZE)						:: r_dofs			!< DoFs describing the function

	!For linear and constant basis functions: DoFs = values
	r_dofs(1 : min(3, _BF_SIZE)) = r_values(1 : min(3, _BF_SIZE))

	!Edge DoFs are computed from the values of the respective edge and its adjacent nodes
#	if (_BF_ORDER > 1)
		r_dofs(_BF_EDGE1 : _BF_EDGE1E) = &
			MATMUL(m_phi_inv(_BF_EDGE1 : _BF_EDGE1E, _BF_EDGE1 : _BF_EDGE1E), r_values(_BF_EDGE1 : _BF_EDGE1E)) + &
			m_phi_inv(_BF_EDGE1 : _BF_EDGE1E, 2) * r_values(2) + &
			m_phi_inv(_BF_EDGE1 : _BF_EDGE1E, 3) * r_values(3)

		r_dofs(_BF_EDGE2 : _BF_EDGE2E) = &
			MATMUL(m_phi_inv(_BF_EDGE2 : _BF_EDGE2E, _BF_EDGE2 : _BF_EDGE2E), r_values(_BF_EDGE2 : _BF_EDGE2E)) + &
			m_phi_inv(_BF_EDGE2 : _BF_EDGE2E, 1) * r_values(1) + &
			m_phi_inv(_BF_EDGE2 : _BF_EDGE2E, 3) * r_values(3)

		r_dofs(_BF_EDGE3 : _BF_EDGE3E) = &
			MATMUL(m_phi_inv(_BF_EDGE3 : _BF_EDGE3E, _BF_EDGE3 : _BF_EDGE3E), r_values(_BF_EDGE3 : _BF_EDGE3E)) + &
			m_phi_inv(_BF_EDGE3 : _BF_EDGE3E, 1) * r_values(1) + &
			m_phi_inv(_BF_EDGE3 : _BF_EDGE3E, 2) * r_values(2)
#	endif

	!Higher-order cell DoFs depend on all values
#	if (_BF_ORDER > 2)
		r_dofs(_BF_CELL :) = MATMUL(m_phi(:, _BF_CELL :), r_values)
#	endif
end function
...
#	elif (_BF_ORDER == 2)
		...
		r_dofs(4) = r_dofs2(5)
		r_dofs(5) = r_dofs1(2) - 0.5_GRID_SR * r_dofs1(1) - 0.5_GRID_SR * r_dofs2(3)
		r_dofs(6) = r_dofs1(5)
#	else
#		error: Hierarchical basis must be of order less than 3
#	endif
...
#	elif (_BF_ORDER == 2)
		...
		r_dofs1(2) = DOT_PRODUCT(_PSI(0.5_GRID_SR, 0.5_GRID_SR), r_dofs)
		r_dofs1(3) = r_dofs(2)
		r_dofs1(4) = DOT_PRODUCT(_PSI(0.25_GRID_SR, 0.25_GRID_SR) - 0.5_GRID_SR * (_PSI(0.0_GRID_SR, 0.0_GRID_SR) + _PSI(0.5_GRID_SR, 0.5_GRID_SR)), r_dofs)
		r_dofs1(5) = r_dofs(6)
		r_dofs1(6) = DOT_PRODUCT(_PSI(0.75_GRID_SR, 0.25_GRID_SR) - 0.5_GRID_SR * (_PSI(0.5_GRID_SR, 0.5_GRID_SR) + _PSI(1.0_GRID_SR, 0.0_GRID_SR)), r_dofs)

		r_dofs2(1) = r_dofs(2)
		r_dofs2(2) = r_dofs1(2)
		r_dofs2(3) = r_dofs(3)
		r_dofs2(4) = DOT_PRODUCT(_PSI(0.25_GRID_SR, 0.75_GRID_SR) - 0.5_GRID_SR * (_PSI(0.5_GRID_SR, 0.5_GRID_SR) + _PSI(0.0_GRID_SR, 1.0_GRID_SR)), r_dofs)
		r_dofs2(5) = r_dofs(4)
		r_dofs2(6) = r_dofs1(4)
#	else
#		error: Hierarchical basis must be of order less than 3
...
#	elif (_BF_ORDER == 2)
		...
		r_dofs1(2) = DOT_PRODUCT(_PSI(0.5_GRID_SR, 0.5_GRID_SR), r_dofs)
		r_dofs1(3) = r_dofs(2)
		r_dofs1(4) = lookup(MATMUL(r_mat_transform, [ 0.25_GRID_SR, 0.25_GRID_SR ]) + r_offset_transform) - 0.5_GRID_SR * DOT_PRODUCT(_PSI(0.0_GRID_SR, 0.0_GRID_SR) + _PSI(0.5_GRID_SR, 0.5_GRID_SR), r_dofs)
		r_dofs1(5) = r_dofs(6)
		r_dofs1(6) = lookup(MATMUL(r_mat_transform, [ 0.75_GRID_SR, 0.25_GRID_SR ]) + r_offset_transform) - 0.5_GRID_SR * DOT_PRODUCT(_PSI(0.5_GRID_SR, 0.5_GRID_SR) + _PSI(1.0_GRID_SR, 0.0_GRID_SR), r_dofs)

		r_dofs2(1) = r_dofs(2)
		r_dofs2(2) = r_dofs1(2)
		r_dofs2(3) = r_dofs(3)
		r_dofs2(4) = lookup(MATMUL(r_mat_transform, [ 0.25_GRID_SR, 0.75_GRID_SR ]) + r_offset_transform) - 0.5_GRID_SR * DOT_PRODUCT(_PSI(0.5_GRID_SR, 0.5_GRID_SR) + _PSI(0.0_GRID_SR, 1.0_GRID_SR), r_dofs)
		r_dofs2(5) = r_dofs(4)
		r_dofs2(6) = r_dofs1(4)
#	else
#		error: Hierarchical basis must be of order less than 3
...
subroutine _BF_(test)()
	...
	!test for dofs -> values transformation correctness

	r_values = _BF_(dofs_to_values)(r_dofs)
	do i = 1, _BF_SIZE
		assert_eq(r_values(i), _BF_(eval)(r_dof_coords(:, i), r_dofs))
	end do

	!test for values -> dofs transformation correctness

	r_values = _BF_(values_to_dofs)(r_values)
	do i = 1, _BF_SIZE
...
#		elif (_BF_ORDER == 2)
			r_value = r_value + (0.75_GRID_SR * r_dofs(2) + 0.25_GRID_SR * r_dofs(1) + 0.75_GRID_SR * r_dofs(6))
#		endif
	end do
