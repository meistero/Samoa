...
#   define _GT_NAME                     t_FLASH_heun1_timestep_traversal
...
#   include "SFC_generic_traversal_ringbuffer.f90"
	...
	subroutine pre_traversal_grid_op(traversal, grid)
			type(t_flash_heun1_timestep_traversal), intent(inout)		:: traversal
...
!#     endif
    ...
    subroutine skeleton_array_op(traversal, grid, edges, rep1, rep2, update1, update2)
      type(t_FLASH_heun1_timestep_traversal), intent(in)        :: traversal
    ...
    subroutine bnd_skeleton_scalar_op(traversal, grid, edge, rep, update)
      ...
      bnd_rep%p= 0.0
      bnd_rep%b= rep%Q(1)%b
      bnd_rep = t_state([0.0,0.0], reshape((/0.0, 0.0 , 0.0, 0.0/),shape(t_state%p)), rep%Q(1)%b)
	!reshape((/ 1, 2, 3, 4, 5, 6, 7, 8, 9 /), shape(array))
      r_minh_r = 1
      r_minh_l = 1
      r_h_l  = rep%Q(:)%h(1)
      r_hu_l = rep%Q(:)%p(1,1)
    ...
    subroutine cell_update_op(traversal, grid, element, update1, update2, update3)
      ...
      type(t_state), dimension(_FLASH_CELL_SIZE)                :: dQ,Q_temp

      call volume_op(traversal, grid, element, dQ, [update1%flux, update2%flux, update3%flux])
	call gv_Q%read(element, Q_temp)
      _log_write(0, *) "dQ: ",dQ
      dQ(:)%h(2)    = dQ(:)%h(2)  + Q_temp(:)%h(1)
      dQ(:)%p(1,2)   = dQ(:)%p(1,2) + Q_temp(:)%p(1,1)
      dQ(:)%p(2,2)   = dQ(:)%p(2,2) + Q_temp(:)%p(2,1)
      ...
      call gv_Q%add(element, dQ)
		...
		subroutine cell_last_touch_op(traversal, grid, cell)
			...
			b_norm = minval(abs(cell%data_pers%Q%h(1) - cell%data_pers%Q%b))
    ...
    subroutine volume_op(traversal, section, element, dQ, fluxes)
      ...
      dQ(:)%h(2)    = ((edge_lengths(1) * fluxes(1)%h(2) + edge_lengths(2) * fluxes(2)%h(2) + edge_lengths(3) * fluxes(3)%h(2)))* (-section%r_dt / volume)
      !dQ(:)%h(2)    = sum(edge_lengths * fluxes%h(2))* (-section%r_dt / volume)
      dQ(:)%p(1,2) = ((edge_lengths(1) * fluxes(1)%p(1,2) + edge_lengths(2) * fluxes(2)%p(1,2) + edge_lengths(3) * fluxes(3)%p(1,2)))* (-section%r_dt / volume)
      !dQ(:)%p(1,2) = sum(edge_lengths * fluxes%p(1,2))* (-section%r_dt / volume)
      dQ(:)%p(2,2) = ((edge_lengths(1) * fluxes(1)%p(2,2) + edge_lengths(2) * fluxes(2)%p(2,2) + edge_lengths(3) * fluxes(3)%p(2,2)))* (-section%r_dt / volume)
      !dQ(:)%p(2,2) = sum(edge_lengths * fluxes%p(2,2))* (-section%r_dt / volume)


      dQ(:)%b    = 0.0_GRID_SR
      dQ(:)%h(1)    = 0.0_GRID_SR
      dQ(:)%p(1,1) = 0.0_GRID_SR
      dQ(:)%p(2,1) = 0.0_GRID_SR


!	_log_write(0, *) "STATE VARIABLES- SECOND STAGE: ", dQ(:)%h(2),dQ(:)%p(1,2),dQ(:)%p(2,2)

!	_log_write(0, *) "STATE VARIABLES- FIRST STAGE: ", dQ(:)%h(1),dQ(:)%p(1,1),dQ(:)%p(1,2)
      !set refinement condition

      element%cell%geometry%refinement = 0
      dQ_norm = dot_product(dQ(1)%p(:,1), dQ(1)%p(:,1))
...
!> Augmented Riemann solver from geoclaw
		subroutine compute_geoclaw_flux(normal, QL, QR, fluxL, fluxR)
			...
			pL = matmul(transform_matrix, QL%p(:,1))
			pR = matmul(transform_matrix, QR%p(:,1))
			hL = QL%h(1) - QL%b
			hR = QR%h(1) - QR%b
			bL = QL%b
			bR = QR%b
...
#           endif
			...
			fluxL%h(2) = net_updatesL(1)
			fluxL%p(:,2) = matmul(net_updatesL(2:3), transform_matrix)
			fluxL%max_wave_speed = max_wave_speed

			fluxR%h(2) = net_updatesR(1)
			fluxR%p(:,2) = matmul(net_updatesR(2:3), transform_matrix)
...
#   define _GT_NAME                     t_FLASH_heun2_timestep_traversal
...
#   include "SFC_generic_traversal_ringbuffer.f90"
	...
	subroutine pre_traversal_grid_op(traversal, grid)
			type(t_flash_heun2_timestep_traversal), intent(inout)		:: traversal
			type(t_grid), intent(inout)							    :: grid

        grid%r_dt = 0.45_GRID_SR * cfg%scaling * get_edge_size(grid%d_max) / ((2.0_GRID_SR + sqrt(2.0_GRID_SR)) * grid%u_max)
	!print *, "Calculated dt in second: ", grid%r_dt, cfg%scaling, grid%u_max,get_edge_size(grid%d_max)
	!print *, "calculatzed Dt: ",0.45_GRID_SR * grid%scaling * get_edge_size(grid%d_max) / ((2.0_GRID_SR + sqrt(2.0_GRID_SR)) * grid%u_max), grid%r_dt
...
#           endif
	...
	!grid%u_max = 0.0_GRID_SR
	!grid%r_dt = grid%r_dt / 2.0_GRID_SR
	end subroutine
    ...
    function cell_to_edge_op(element, edge) result(rep)
      ...
      _log_write(6, '(4X, A, F0.3, 1X, F0.3, 1X, F0.3, 1X, F0.3)') "Q in: ", Q
            _log_write(6, '(4X, A, F0.3, 1X, F0.3)') "normal in : ", edge%transform_data%normal
!_log_write(0, *) "element_orientation: ", element%transform_data%plotter_data%orientation
!#     if (_FLASH_CELL_SIZE > 1)
      !  i_edge = edge%transform_data%index
  !      _log_write(6, '(4X, A, I0)') "edge ", i_edge
	!print *, "-------------------------------------------"
...
!#     endif
    ...
    subroutine skeleton_array_op(traversal, grid, edges, rep1, rep2, update1, update2)
      type(t_FLASH_heun2_timestep_traversal), intent(in)        :: traversal
      type(t_grid_section), intent(in)                          :: grid
      type(t_edge_data), intent(in)                             :: edges(:)
      type(num_cell_rep), intent(in)                            :: rep1(:), rep2(:)
      type(num_cell_update), intent(out)                        :: update1(:), update2(:)
	...
	!update2%flux(:)%max_wave_speed = max_wave_speed
      _log_write(6, '(4X, A, F0.3, 1X, F0.3, 1X, F0.3, 1X, F0.3)') "flux 1 out: ", update1%flux
      _log_write(6, '(4X, A, F0.3, 1X, F0.3, 1X, F0.3, 1X, F0.3)') "flux 2 out: ", update2%flux
     ! _log_write(0, '(4X, A, 4(X, F0.3))') "1 flux in:", update1%flux
     ! _log_write(0, '(4X, A, 4(X, F0.3))') "2 flux in:", update2%flux
    end subroutine
...
!_log_write(0, *) "boundary CELL "
    ...
    subroutine cell_update_op(traversal, grid, element, update1, update2, update3)
      ...
      type(t_state), dimension(_FLASH_CELL_SIZE)                :: dQ,dQ_temp
      type(t_state), dimension(_FLASH_CELL_SIZE)                :: Q_temp
!      _log_write(0, '(4X, A, 4(X, F0.3))') "edge 1 flux in:", update1%flux
!      _log_write(0, '(4X, A, 4(X, F0.3))') "edge 2 flux in:", update2%flux
!      _log_write(0, '(4X, A, 4(X, F0.3))') "edge 3 flux in:", update3%flux
      call volume_op(traversal, grid, element, dQ, [update1%flux, update2%flux, update3%flux])
      ...
      !_log_write(0, *) "test: " , dQ_temp%h(1) , Q_temp%h(1), Q_temp%h(2)
      dQ_temp(:)%h(1)    = (Q_temp(:)%h(1) + Q_temp(:)%h(2))/2.0_GRID_SR
      dQ_temp(:)%p(1,1) = (Q_temp(:)%p(1,1) + Q_temp(:)%p(1,2))/2.0_GRID_SR
      dQ_temp(:)%p(2,1) = (Q_temp(:)%p(2,1) + Q_temp(:)%p(2,2))/2.0_GRID_SR
      dQ_temp(:)%b = Q_temp%b
      dQ_temp(:)%h(2)    = dQ_temp(:)%h(1)
      dQ_temp(:)%p(1,2) = dQ_temp(:)%p(1,1)
      dQ_temp(:)%p(2,2) = dQ_temp(:)%p(2,1)
     ! _log_write(0, *) "   TESING1: ",ieee_is_finite(dQ_temp%h(1)), dQ_temp%h(1)!IEEE_SUPPORT_DATATYPE(dQ_temp%h(1)) 
      if ((.not. (ieee_is_finite(Q_temp(1)%h(1)))) .or. (ieee_is_nan(Q_temp(1)%h(1)))) then
_log_write(0, *) "   NOT DEFINED STATE VARIABLE: ", (.not. ieee_is_finite(dQ_temp%h(1))),ieee_is_nan(dQ_temp(1)%h(1)),dQ(1)%h(1),dQ(1)%h(2),dQ_temp(1)%h(1),dQ_temp(1)%h(2)
      dQ_temp%h(1)    = 0.0_GRID_SR
      dQ_temp%p(1,1) = 0.0_GRID_SR
      dQ_temp%p(2,1) = 0.0_GRID_SR
	pause
	endif
!_log_write(0, *) "element_orientation: ", element%transform_data%plotter_data%orientation
      call gv_Q%write(element,dQ_temp)
      call gv_Q%read(element, Q_temp)
      if((Q_temp(1)%h(1)) .ne. (dQ_temp(1)%h(1))) then
	  ! _log_write(0, *) "   TESING1: ",Q_temp%h(1), dQ_temp%h(1)
	  ! _log_write(0, *) "   TESING2: ",Q_temp%h(2), dQ_temp%h(2)
	  ! _log_write(0, *) "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
      endif
		...
		subroutine cell_last_touch_op(traversal, grid, cell)
			...
			depth = cell%geometry%i_depth
			b_norm = minval(abs(cell%data_pers%Q%h(2) - cell%data_pers%Q%b))
    ...
    subroutine volume_op(traversal, section, element, dQ, fluxes)
      ...
      dQ(:)%h(1)    = ( (edge_lengths(1) * fluxes(1)%h(1) + edge_lengths(2) * fluxes(2)%h(1) + edge_lengths(3) * fluxes(3)%h(1)))* (-section%r_dt / volume)
      dQ(:)%p(1,1) = ( (edge_lengths(1) * fluxes(1)%p(1,1) + edge_lengths(2) * fluxes(2)%p(1,1) + edge_lengths(3) * fluxes(3)%p(1,1)))* (-section%r_dt / volume)
      dQ(:)%p(2,1) = ( (edge_lengths(1) * fluxes(1)%p(2,1) + edge_lengths(2) * fluxes(2)%p(2,1) + edge_lengths(3) * fluxes(3)%p(2,1)))* (-section%r_dt / volume)
      !_log_write(0, *) "dQ%h" , dQ(:)%h(1), "Edge_Lengths" , edge_lengths
     ! dQ(:)%h(1)    = sum(edge_lengths * fluxes%h(1))* (-section%r_dt / volume)
     ! dQ(:)%p(1,1) = sum(edge_lengths * fluxes%p(1,1))* (-section%r_dt / volume)
     ! dQ(:)%p(2,1) = sum(edge_lengths * fluxes%p(2,1))* (-section%r_dt / volume)
      dQ(:)%b    = 0.0_GRID_SR
      dQ(:)%h(2)    = 0.0_GRID_SR
      dQ(:)%p(1,2) = 0.0_GRID_SR
      dQ(:)%p(2,2) = 0.0_GRID_SR
      !set refinement condition

      element%cell%geometry%refinement = 0
      dQ_norm = dot_product(dQ(1)%p(:,2), dQ(1)%p(:,2))
      ...
      dQ%h(1)    = (dQ%h(1) + dQ%h(2))/2
      dQ%p(1,1) = (dQ%p(1,1) + dQ%p(1,2))/2
      dQ%p(2,1) = (dQ%p(2,1) + dQ%p(2,2))/2
...
!> Augmented Riemann solver from geoclaw
		subroutine compute_geoclaw_flux(normal, QL, QR, fluxL, fluxR)
			...
			pL = matmul(transform_matrix, QL%p(:,2))
			pR = matmul(transform_matrix, QR%p(:,2))
			hL = QL%h(2) - QL%b
			hR = QR%h(2) - QR%b
			bL = QL%b
			bR = QR%b
...
#           endif
	...
	if ((.not. (ieee_is_finite(net_updatesL(1)))) .or. (ieee_is_nan(net_updatesL(1)))) then
		      _log_write(0, '(4X, A, 4(X, F0.3))') "update R flux in:", hL, hR, pL(1), g
		      _log_write(0, '(4X, A, 4(X, F0.3))') "update L flux in:", pR(1), pL(2), pR(2), bL
	endif


			fluxL%h(1) = net_updatesL(1)
			fluxL%p(:,1) = matmul(net_updatesL(2:3), transform_matrix)
			fluxL%max_wave_speed = max_wave_speed

			fluxR%h(1) = net_updatesR(1)
			fluxR%p(:,1) = matmul(net_updatesR(2:3), transform_matrix)
			fluxR%max_wave_speed = max_wave_speed
    !  _log_write(0, '(4X, A, 4(X, F0.3))') "riemann R flux in:", fluxR
    !  _log_write(0, '(4X, A, 4(X, F0.3))') "riemann L flux in:", fluxL
		end subroutine
