c-----------------------------------------------------------------------
     ...
     &   hvL,hvR,bL,bR,uL,uR,vL,vR,delphi,sE1,sE2,drytol,g,sw,fw)
      ...
      real (kind = QP) huRstar,huLstar,uRstar,uLstar,hstarHLL
      real (kind = QP) deldelh,deldelphi
      real (kind = QP) s1m,s2m,hm
      real (kind = QP) det1,det2,det3,determinant
      ...
      call riemanntype_qp(hL,hR,uL,uR,hm,s1m,s2m,rare1,rare2,
     &                                          1,drytol,g)
      ...
      sE1=lambda(1)
      sE2=lambda(3)
      hstarHLL = max((huL-huR+sE2*hR-sE1*hL)/(sE2-sE1),0.q0) ! middle state in an HLL solve
...
c     !determine the middle entropy corrector wave------------------------
      ...
      if (rarecorrectortest) then
         ...
         raremin = 0.5q0
         raremax = 0.9q0
         if (rare1.and.sE1*s1m.lt.0.q0) raremin=0.2q0
         if (rare2.and.sE2*s2m.lt.0.q0) raremin=0.2q0
         if (rare1.or.rare2) then
            !see which rarefaction is larger
            rare1st=3.q0*(sqrt(g*hL)-sqrt(g*hm))
            rare2st=3.q0*(sqrt(g*hR)-sqrt(g*hm))
     ...
     &         max(rare1st,rare2st).lt.raremax*sdelta) then
               ...
               elseif (rare2st.gt.rare1st) then
                  ...
                  lambda(2)=0.5q0*(s1m+s2m)
               endif
            endif
         endif
         if (hstarHLL.lt.min(hL,hR)/5.q0) rarecorrector=.false.
      endif

      if (abs(lambda(2)) .lt. 1.q-20) lambda(2) = 0.q0
      do mw=1,mwaves
         r(1,mw)=1.q0
         r(2,mw)=lambda(mw)
         r(3,mw)=(lambda(mw))**2
      ...
      if (.not.rarecorrector) then
         lambda(2) = 0.5q0*(lambda(1)+lambda(3))
c         lambda(2) = max(min(0.5q0*(s1m+s2m),sE2),sE1)
         if (abs(lambda(2)) .lt. 1.q-20) lambda(2) = 0.q0
         r(1,2)=0.q0
         r(2,2)=0.q0
         r(3,2)=1.q0
...
c           lambda(2) = max(min(0.5q0*(s1m+s2m),sE2),sE1)
         ...
         hbar =  max(0.5q0*(hLstar+hRstar),0.q0)
         s1s2bar = 0.25q0*(uLstar+uRstar)**2 - g*hbar
         s1s2tilde= max(0.q0,uLstar*uRstar) - g*hbar
...
c        !find if sonic problem
         ...
         if (sE1.lt.0.q0.and.s1m.gt.0.q0) sonic = .true.
         if (sE2.gt.0.q0.and.s2m.lt.0.q0) sonic = .true.
         if ((uL+sqrt(g*hL))*(uR+sqrt(g*hR)).lt.0.q0) sonic=.true.
         if ((uL-sqrt(g*hL))*(uR-sqrt(g*hR)).lt.0.q0) sonic=.true.
...
c        !solve for beta(k) using Cramers Rule=================
      ...
      end !subroutine riemann_aug_JCP_qp-------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     &    hvL,hvR,bL,bR,uL,uR,vL,vR,delphi,sE1,sE2,drytol,g,sw,fw)
      ...
      real (kind = QP) deldelh,deldelphi
      real (kind = QP) alpha1,alpha2,beta1,beta2,delalpha1,delalpha2
      real (kind = QP) criticaltol,convergencetol
      real (kind = QP) sL,sR
      real (kind = QP) uhat,chat,sRoe1,sRoe2
...
!
            if (sE2.gt.0.q0.and.sE1.lt.0.q0) then
               hLstar=hL+alpha1
               hRstar=hR-alpha2
c               hustar=huL+alpha1*sE1
               hustar = huL + beta1
            elseif (sE1.ge.0.q0) then
               hLstar=hL
               hustar=huL
               hRstar=hR - alpha1 - alpha2
            elseif (sE2.le.0.q0) then
               hRstar=hR
               hustar=huR
...
c=============================================================================
      subroutine riemanntype_qp(hL,hR,uL,uR,hm,s1m,s2m,rare1,rare2,
     &             maxiter,drytol,g)
      ...
      real (kind = QP) hL,hR,uL,uR,drytol,g
      integer maxiter

      !output
      real (kind = QP) s1m,s2m
      logical rare1,rare2

      !local
      real (kind = QP) hm,u1m,u2m,um,delu
      real (kind = QP) h_max,h_min,h0,F_max,F_min,dfdh,F0,slope,gL,gR
      integer iter
...
c     !Test for Riemann structure
      ...
      if (h_min.le.drytol) then
         hm=0.q0
         um=0.q0
         s1m=uR+uL-2.q0*sqrt(g*hR)+2.q0*sqrt(g*hL)
         s2m=uR+uL-2.q0*sqrt(g*hR)+2.q0*sqrt(g*hL)
         if (hL.le.0.q0) then
            rare2=.true.
            rare1=.false.
         else
         ...
         F_min= delu+2.q0*(sqrt(g*h_min)-sqrt(g*h_max))
         F_max= delu +
     &         (h_max-h_min)*(sqrt(.5q0*g*(h_max+h_min)/(h_max*h_min)))

         if (F_min.gt.0.q0) then !2-rarefactions

            hm=(1.q0/(16.q0*g))*
     &               max(0.q0,-delu+2.q0*(sqrt(g*hL)+sqrt(g*hR)))**2
            um=sign(1.q0,hm)*(uL+2.q0*(sqrt(g*hL)-sqrt(g*hm)))

            s1m=uL+2.q0*sqrt(g*hL)-3.q0*sqrt(g*hm)
            s2m=uR-2.q0*sqrt(g*hR)+3.q0*sqrt(g*hm)

            rare1=.true.
            rare2=.true.

         elseif (F_max.le.0.q0) then !2 shocks

c           !root finding using a Newton iteration on sqrt(h)===
            h0=h_max
            do iter=1,maxiter
               gL=sqrt(.5q0*g*(1/h0 + 1/hL))
               gR=sqrt(.5q0*g*(1/h0 + 1/hR))
               F0=delu+(h0-hL)*gL + (h0-hR)*gR
               dfdh=gL-g*(h0-hL)/(4.q0*(h0**2)*gL)+
     &                   gR-g*(h0-hR)/(4.q0*(h0**2)*gR)
               slope=2.q0*sqrt(h0)*dfdh
               h0=(sqrt(h0)-F0/slope)**2
            enddo
               hm=h0
               u1m=uL-(hm-hL)*sqrt((.5q0*g)*(1/hm + 1/hL))
               u2m=uR+(hm-hR)*sqrt((.5q0*g)*(1/hm + 1/hR))
               um=.5q0*(u1m+u2m)
            ...
            do iter=1,maxiter
               F0=delu + 2.q0*(sqrt(g*h0)-sqrt(g*h_max))
     &                  + (h0-h_min)*sqrt(.5q0*g*(1/h0+1/h_min))
            ...
            hm=h0
            if (hL.gt.hR) then
               um=uL+2.q0*sqrt(g*hL)-2.q0*sqrt(g*hm)
               s1m=uL+2.q0*sqrt(g*hL)-3.q0*sqrt(g*hm)
               s2m=uL+2.q0*sqrt(g*hL)-sqrt(g*hm)
               rare1=.true.
               rare2=.false.
            else
               s2m=uR-2.q0*sqrt(g*hR)+3.q0*sqrt(g*hm)
               s1m=uR-2.q0*sqrt(g*hR)+sqrt(g*hm)
               um=uR-2.q0*sqrt(g*hR)+2.q0*sqrt(g*hm)
               rare2=.true.
               rare1=.false.
            endif
         endif
